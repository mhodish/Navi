const CACHE='roadbook-v18-9-6';
const LOCAL=['./','./index.html','./manifest.json','./roadbook-icon-192.png','./roadbook-icon-512.png'];
...
self.addEventListener('activate',event=>event.waitUntil((async()=>{
  ...
  await Promise.all(keys.filter(k=>(k.startsWith('europe-roadbook-v')||k.startsWith('roadbook-v'))&&k!==CACHE).map(k=>caches.delete(k)));
